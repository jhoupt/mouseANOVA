minimum_jerk_policy <- function(initial_state, final_state, D, 
  ...
  qdot$x <- Q %*% initial_state$x + df * final_state$x
  qdot$y <- Q %*% initial_state$y + df * final_state$y


   
  halt_policy <- (abs(initial_state$x[1] - final_state$x) < x_tol & 
                  abs(initial_state$y[1] - final_state$y) < y_tol & 
